...
class TaskViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(task: Task, onTaskClick: (Task) -> Unit, onDeleteTask: (Task) -> Unit) {
        setupBasicInfo(task)
        setupDateTimeInfo(task)
        setupListeners(task, onTaskClick, onDeleteTask)
    }

    private fun setupBasicInfo(task: Task) {
        titleTextView.text = task.title
        descriptionTextView.text = task.description
        priorityTextView.text = task.priority
    }

    private fun setupDateTimeInfo(task: Task) {
    ...
    private fun setupListeners(task: Task, onTaskClick: (Task) -> Unit, onDeleteTask: (Task) -> Unit) {
